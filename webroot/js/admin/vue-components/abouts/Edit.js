app.component('edit-abouts', {
    template: document.getElementById('edit-abouts').innerHTML,
    data() {
        return {
            formData: {},
            formLoading: true,
    ...
    methods: {
        async onSubmit(data) {
            ...
            const resp = await saveData('Abouts', {id: getId(),...data, ...media});
            if (resp.success) {
        ...
        async getRecord() {
            try {
                ...
                const resp = await getOne('Abouts', { 
                    where: {
                        'Abouts.id': getId()
                    },
                    contain: ['Media']
